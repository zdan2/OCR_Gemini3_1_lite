...
Examples:
  # Full 13-page batch over the 4 default Gemini models, fresh run (cost data captured):
  python .\vision_ocr_shootout_v2.py --force

  # Cheaper smoke test on 5 pages:
  python .\vision_ocr_shootout_v2.py --force --max-pages 5

  # Explicitly pick the 4 Gemini finalists (same as default):
  python .\vision_ocr_shootout_v2.py --force --models gemini_3_5_flash,gemini_3_1_pro_preview,gemini_2_5_flash,gemini_3_1_flash_lite

  # Bring back de-selected models (Claude / GPT / lite tiers) when needed:
  python .\vision_ocr_shootout_v2.py --force --models claude_sonnet_4_6,gpt_5_4
...
# Default to all pages: this build is meant for a full 13-page batch run.
# Use --max-pages 5 for a cheaper smoke test.
...
MODELS: list[ModelSpec] = [
    ModelSpec(
        ...
        notes="Claude balanced/high quality (run via --models)",
    ),
    ModelSpec(
        ...
        enabled_by_default=False,
        notes="Claude highest quality; temperature omitted (run via --models)",
    ),
    ModelSpec(
    ...
    ModelSpec(
        ...
        enabled_by_default=False,
        notes="Gemini 3 Flash preview; previously truncated; run via --models",
    ),
    ModelSpec(
    ...
    ModelSpec(
        ...
        enabled_by_default=False,
        notes="Very cheap baseline; misread-prone; run via --models",
    ),
    ModelSpec(
        label="gpt_5_5",
        provider="openai",
        ...
        notes="OpenAI flagship; weak on JP handwriting in testing; run via --models",
    ),
    ModelSpec(
        label="gpt_5_4",
        provider="openai",
        ...
        enabled_by_default=False,
        openai_reasoning_effort="low",
        notes="OpenAI cost-quality tier; weak on JP handwriting in testing; run via --models",
    ),
]
...
def usage_to_jsonable(usage: Any) -> Any:
    """Best-effort conversion of an SDK usage object into JSON-safe data.

    OpenAI's usage nests objects like InputTokensDetails / OutputTokensDetails
    that are not directly serialisable, and Gemini/Anthropic usage objects vary
    by SDK version. We try the common shapes, then fall back to str()."""
    if usage is None:
        return None
    # Pydantic v2 models (OpenAI, google-genai) expose model_dump().
    ...
    if hasattr(usage, "__dict__"):
        # Shallow dict may still contain nested objects; stringify them.
        out: dict[str, Any] = {}
...
def run_anthropic(spec: ModelSpec, image_path: Path, page_prompt: str) -> dict[str, Any]:
    ...
    return {
        ...
        "elapsed_sec": elapsed,
        "raw_usage": usage_to_jsonable(usage),
    }
...
def write_score_sheet_template(path: Path, run_rows: list[dict[str, Any]]) -> None:
    ...
    # Sort by page then model so each page's models sit together for side-by-side
    # human scoring against the same source image.
    ok_rows = sorted(ok_rows, key=lambda r: (int(r.get("page") or 0), str(r.get("label", ""))))
    with path.open("w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=fields)
        writer.writeheader()
        for r in ok_rows:
            # error column was reused to flag truncation in process_one_model.
            writer.writerow({
                "pdf": r.get("pdf", ""),
                "page": r.get("page", ""),
                "label": r.get("label", ""),
...
def main() -> None:
    ...
    if not args.force:
        print("")
        print("  NOTE: --force 未指定です。既存のOCR結果Markdownがある場合は")
        print("        cached 扱いとなり、metrics/summary のトークン・コストは")
        print("        記録されません。一次データ（コスト集計）が必要なら --force を付けてください。")
    print("")
