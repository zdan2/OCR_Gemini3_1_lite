...
Setup:
  ...
  pip install -U anthropic google-genai pdf2image pillow
...
def estimate_cost_usd(spec: ModelSpec, input_tokens: Optional[int], output_tokens: Optional[int]) -> Optional[float]:
...
def build_gemini_config(spec: ModelSpec):
    ...
    if spec.gemini_thinking_level:
        # This is optional and left disabled by default for broad model compatibility.
        kwargs["thinking_config"] = genai_types.ThinkingConfig(thinking_level=spec.gemini_thinking_level)
...
def write_score_sheet_template(path: Path, run_rows: list[dict[str, Any]]) -> None:
    ...
    ok_rows = [r for r in run_rows if r.get("status") in {"ok", "cached"}]
...
def process_one_model(
    *,
    ...
    try:
        ...
        estimated_cost = estimate_cost_usd(spec, input_tokens, output_tokens)
