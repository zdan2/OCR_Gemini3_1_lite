...
| 同じ日の連絡帳が二重に入る | watcherが**同じ(日付,クラス)のPDFは1冊だけ処理**し、別名の重複（全角空白/下線/`_001`違い）は「重複スキップ」する。既に二重登録された過去分は手動削除が必要 |
| **全員が要確認・クラス未確定**になる | 名簿Excelに**その月のシートが未追加**（例: 7月分のスキャンが来たのに7月シートが無い）。照合は自動で**直近の既存月シートで代用**する（ログに `[名簿代用]`）ので動きはするが、入園月ズレの恐れがあるため**月初に新しい月のシート入り名簿を `/data` に差し替える**のが正。代用前に取り込まれた行は削除→再取り込みで直す |
